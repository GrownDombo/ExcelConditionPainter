...
Setup 프로젝트는 Visual Studio Installer Projects 확장이 필요합니다.
`main` 브랜치에 push되면 GitHub Actions가 배포 버전을 확인하고, 같은 버전의 GitHub Release가 없을 때만 설치 파일을 빌드해 배포합니다.
...
| 변경 예시 | 수정 파일 |
| --- | --- |
| `1.0.1.0` → `1.0.1.1` | `AssemblyInfo.cs` |
| `1.0.1.0` → `1.0.2.0` | `AssemblyInfo.cs`, `ExcelConditionPainter.Setup.vdproj` |

MSI의 `ProductVersion`은 3자리 버전을 사용하므로, 앞 3자리가 바뀌는 배포에서는 Setup 프로젝트 버전도 함께 맞춰야 합니다.
